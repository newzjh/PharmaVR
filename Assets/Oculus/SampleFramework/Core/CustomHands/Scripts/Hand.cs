...
namespace OVRTouchSample
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            //m_controller.activateAction.action.performed += OnInputFocusAcquired;
        ...
        private void Update()
        {
            ...
            bool collisionEnabled = false;// m_grabber.grabbedObject == null && flex >= THRESH_COLLISION_FLEX;
        ...
        {
            m_isPointing = false;// !OVRInput.Get(OVRInput.NearTouch.PrimaryIndexTrigger, m_controller);
            m_isGivingThumbsUp = false;// !OVRInput.Get(OVRInput.NearTouch.PrimaryThumbButtons, m_controller);
        }
        ...
        public void OnInputFocusLost(InputAction.CallbackContext callbackcontext)
        {
        ...
        public void OnInputFocusAcquired(InputAction.CallbackContext callbackcontext)
        {
        ...
        public float flex = 0.0f;
        public float pinch = 0.0f;
        private void UpdateAnimStates()
        {
            bool grabbing = m_controller.selectAction.action.triggered;// m_grabber.grabbedObject != null;
            ...
            if (m_controller.selectAction.action.controls.Count>0)
                flex += m_controller.selectAction.action.controls[0].IsPressed() ? 0.02f : -0.02f;
            flex = Mathf.Clamp01(flex);
            //float flex = m_controller.activateAction.action.triggered?1:0;// OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, m_controller);
            ...
            if (m_controller.activateAction.action.controls.Count > 0)
                pinch += m_controller.activateAction.action.controls[0].IsPressed() ? 0.02f : -0.02f;
            pinch = Mathf.Clamp01(pinch);
            //float pinch = m_controller.selectAction.action.triggered?1:0;// OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger, m_controller);
