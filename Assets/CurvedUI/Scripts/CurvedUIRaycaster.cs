	...
	public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList) {
        ...
        switch (mySettings.Controller)
        {
            case CurvedUISettings.CurvedUIController.MOUSE:
            {
				...
				if (screenpos.x>=-Screen.width && screenpos.x<=Screen.width*2 && screenpos.y>=-Screen.height && screenpos.y<= Screen.height*2)
					ray3D = worldCamera.ScreenPointToRay(screenpos);
				else
