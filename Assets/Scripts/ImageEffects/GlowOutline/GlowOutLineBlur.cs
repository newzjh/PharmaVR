...
namespace ImageEffects
{
    internal class GlowOutLineBlur : BlurBase, IBlur
    {
        ...
        {
            Shader shader = Shader.Find("ImageEffect/Glow/Compose Max");
            if (!shader.isSupported)
            {
                shader = Shader.Find("ImageEffect/Glow/Compose Max Fallback");
                maxFallback = true;
            }
            composeMaxMaterial = new Material(shader);
            composeMaxMaterial.hideFlags = HideFlags.DontSave;

            Shader BlendShader = Shader.Find("ImageEffect/Glow/GlowOutLine/BlendToScreen_Simple");
        ...
        public void BlurAndBlitBuffer(RenderTexture rbuffer, RenderTexture destination, GlowSettings settings, bool highPrecision, bool bOptimitzed = true)
        {
            ...
            else
            {
                ...
                BlendToScreen.SetTexture("_SrcTex", rbuffer);
                BlendToScreen.SetTexture("_DstTex", texture4);

                Graphics.Blit(texture4, destination, BlendToScreen);
            }
