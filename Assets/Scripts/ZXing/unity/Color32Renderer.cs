         ...
         for (int y = 0; y < matrix.Height; y++)
         {
            ...
            for (int x = 0; x < bits.Length; x++)
            {
               for (int i = 0; i < 32; i++)
               {
                  int bit = (bits[x] >> i) & 1;
                  if (bit == 1)
                      result[stride * y + x * 32 + i] = new Color32(foreground.r, foreground.g, foreground.b, foreground.a);
                  else
                      result[stride * y + x * 32 + i] = new Color32(background.r, background.g, background.b, background.a);
               }
            }
         }
