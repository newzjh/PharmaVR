...
public class GetGPS : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(20, 600, 600, 48), this.gps_info);
        GUI.Label(new Rect(20, 650, 600, 48), this.flash_num.ToString());

        GUI.skin.button.fontSize = 50;
        //if (GUI.Button(new Rect(Screen.width / 2 - 110, 200, 220, 85), "GPS定位"))
        if (GUI.Button(new Rect(20, 700, 220, 48), "GPS定位"))
        {
            // 这里需要启动一个协同程序  
            StartCoroutine(StartGPS());
        }
        //if (GUI.Button(new Rect(Screen.width / 2 - 110, 500, 220, 85), "刷新GPS"))
        //if (GUI.Button(new Rect(20, 800, 220, 48), "刷新GPS"))
        {
            this.gps_info = "N:" + Input.location.lastData.latitude.ToString("f10") + " E:" + Input.location.lastData.longitude.ToString("f10");
