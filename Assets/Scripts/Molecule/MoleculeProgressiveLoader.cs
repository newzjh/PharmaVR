...
namespace MoleculeLogic
{
    public class MoleculeProgressiveLoader
    {
        ...
        public static IEnumerator LoadFromStream(Molecule mol, Stream stream, string ext)
        {
            if (ext == ".pdb" || ext == ".ent" || ext==".pdbqt" || ext==".ret")
            {
                yield return mol.StartCoroutine(LoadFromPDB(mol, stream));
            }
            else if (ext == ".mol2" || ext == ".ml2" || ext == ".sy2")
            {
                yield return mol.StartCoroutine(LoadFromMol2(mol, stream));
            }
            yield return true;
        }

        private static IEnumerator LoadFromMol2(Molecule mol, Stream stream)
        {
            ...
            while (true)
            {
                ...
                else if (curblockname=="atom")
                {
                    ...
                    if (mol.atoms.Count > 0 && mol.atoms.Count % 100 == 0) yield return waitframe;
                ...
                else if (curblockname=="bond")
                {
                    ...
                    if (index1 >= 0 && index1 < mol.atoms.Count && index2 >= 0 && index2 < mol.atoms.Count)
                    {
                            ...
                            !(atom1.bonds.ContainsKey(atom2) || atom2.bonds.ContainsKey(atom1)))
                        {
                            ...
                            if (mol.bonds.Count > 0 && mol.bonds.Count % 100 == 0) yield return waitframe;
                        }
                    }
                }
        ...
        private static IEnumerator LoadFromPDB(Molecule mol, Stream stream)
        {
            int partIndex = 0;
