...
public class Primite7 : BasePrimite {
	...
	public override void createPrimate(LinkParam link,Transform parent)
	{
		...
		{
			buildPrimite (7, 0 , m_parent,Mathf.PI/2.0f,false);
			m_parent.position=link.linkPosition;
		}
		else if (link.linkType==0) //node
		{
			buildPrimite (7, 0 , m_parent,Mathf.PI,false);
		...
		{
			buildPrimite (7, 0 , m_parent,-Mathf.PI*2.0f/7.0f*3.0f,true);
			m_parent.position=link.linkPosition+link.linkNormal*rad*Mathf.Cos(Mathf.PI/7.0f);
			m_parent.right=link.linkNormal;
		}
	}
