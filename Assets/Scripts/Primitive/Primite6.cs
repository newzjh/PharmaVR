...
public class Primite6 : BasePrimite {
	...
	public override void createPrimate(LinkParam link,Transform parent)
	{
		...
		{
			buildPrimite (6, 0 , m_parent,Mathf.PI/2.0f,false);
			m_parent.position=link.linkPosition;
		}
		else if (link.linkType==0) //node
		{
			buildPrimite (6, 0 , m_parent,0,false);
		...
		{
			buildPrimite (6, 0 , m_parent,Mathf.PI/6.0f+Mathf.PI,true);
