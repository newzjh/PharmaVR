...
public class Primite3 : BasePrimite {
	...
	public override void createPrimate(LinkParam link,Transform parent)
	{
		...
		{
			buildPrimite (3, 0 , m_parent,Mathf.PI/2.0f,false);
			m_parent.position=link.linkPosition;
		}
		else if (link.linkType==0) //node
		{
			buildPrimite (3, 0 , m_parent,Mathf.PI/2.0f*3.0f+Mathf.PI/6.0f,false);
		...
		{
			buildPrimite (3, 0 , m_parent,-Mathf.PI/3.0f*2.0f,true);
            m_parent.position = link.linkPosition + link.linkNormal * rad * 0.5f ;
			m_parent.right=link.linkNormal;
		}
	}
