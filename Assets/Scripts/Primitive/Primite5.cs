...
public class Primite5 : BasePrimite {
	...
	public override void createPrimate(LinkParam link,Transform parent)
	{
		...
		{
			buildPrimite (5, 0 , m_parent,Mathf.PI/2.0f,false);
			m_parent.position=link.linkPosition;
		}
		else if (link.linkType==0) //node
		{
			buildPrimite (5, 0 , m_parent,Mathf.PI,false);
		...
		{
			buildPrimite (5, 0 , m_parent,-Mathf.PI*2.0f/5.0f*2.0f,true);
			m_parent.position=link.linkPosition+link.linkNormal*rad*Mathf.Cos(Mathf.PI/5.0f);
			m_parent.right=link.linkNormal;
		}
	}
